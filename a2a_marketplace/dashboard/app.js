// A2A VC Agent Marketplace - Dashboard
...
function renderAgents(agentList) {
    ...
    for (const agent of agentList) {
        ...
        card.innerHTML = `
            ...
            <span class="agent-type ${agent.type}">${(agent.type || '').toUpperCase()}</span>
            <div class="agent-detail">${escapeHtml(agent.description || '')}</div>
            <div class="agent-detail" style="margin-top:0.2rem;color:#484f58">${escapeHtml(agent.url || '')}</div>
        `;
...
function renderDeals(dealList) {
    ...
    for (const deal of dealList) {
        ...
        card.innerHTML = `
            ...
            ${deal.outcome ? `<div class="deal-outcome">${escapeHtml(deal.outcome)}</div>` : ''}
        `;
...
fetchAgents();
fetchDeals();
setInterval(() => { fetchAgents(); fetchDeals(); }, 5000);
